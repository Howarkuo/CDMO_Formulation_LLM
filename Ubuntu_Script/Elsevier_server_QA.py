...
# ================= CONFIGURATION =================
# [SERVER SETUP] Paths adapted for your specific folder structure
KEY_PATH = "api_key.txt"
INPUT_PDF_DIR = "Elsevier_batch_downloads"  # <--- UPDATED to match your ls output
OUTPUT_DIR = "QA_Results_CMC_Elsevier"
...
def setup_client():
    try:
        if not os.path.exists(KEY_PATH):
            print(f"❌ Error: {KEY_PATH} not found in {os.getcwd()}. Please upload it.")
            return None
        with open(KEY_PATH, "r") as f:
            api_key = f.read().strip()
        # Initialize client (ensure you are using the correct Google GenAI SDK version)
...
def save_text_file(data, output_path, mode="Part1", model_used="Unknown"):
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            summary = data.get('brief_summary', 'No summary provided')
            f.write(f"SUMMARY:\n{summary}\n")
            f.write("\n" + "="*50 + "\n")
            f.write("Q&A PAIRS\n")
            f.write("="*50 + "\n\n")
            ...
            for item in data.get('qa_pairs', []):
                ...
                f.write(f"A: {item.get('answer_and_reasoning', '')}\n")
                f.write(f"Source: {item.get('input_paragraph', '')}\n")
                f.write(f"PMID: {item.get('source_pmid', '')}\n")
                f.write("-" * 30 + "\n")
...
def process_single_pdf(client, pdf_path):
    ...
    try:
        ...
        # Note: 'mime_type' is optional but good practice if known
        ...
        # Cleanup (Delete file from Google Server to save space/privacy)
        try:
...
if __name__ == "__main__":
    ...
    # Find PDFs in the INPUT directory
    # Note: If your files do not have .pdf extension, change "*.pdf" to "*"
    pdf_files = glob.glob(os.path.join(INPUT_PDF_DIR, "*.pdf"))
    
    if not pdf_files:
        # Fallback check: sometimes scraped files have no extension
        print(f"⚠️  No .pdf files found in {INPUT_PDF_DIR}. Checking for all files...")
        pdf_files = [f for f in glob.glob(os.path.join(INPUT_PDF_DIR, "*")) if os.path.isfile(f)]

    if not pdf_files:
        print(f"❌ Error: No files found in folder: {INPUT_PDF_DIR}")
        exit()
    ...
    for i, pdf in enumerate(pdf_files, 1):
        ...
        # Sleep to handle rate limits (60 RPM usually)
        time.sleep(10) 
