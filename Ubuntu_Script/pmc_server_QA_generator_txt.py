...
# next step: must split articles by === and sent them one by one 

# ================= CONFIGURATION =================
# [SERVER SETUP]
KEY_PATH = "api_key.txt"
# Directory path
INPUT_TXT_DIR = r"C:\Users\howar\Desktop\biollm\finallist_0108\withpmcid_folder\withpmcid_still_missing_QA\Success_by_VPN"
OUTPUT_DIR = "trial_withpmcidstillmissing_QA_Results_CMC_TXT"
...
def process_single_txt(client, txt_path):
    ...
    try:
        ...
        try:
            ...
            print(f"   ⚠️ UTF-8 Error, trying latin-1...")
        ...
        try:
            res1 = client.models.generate_content(
                model=MODEL_PART1,
                contents=[article_content, PROMPT_PART1],
        ...
        try:
            res2 = client.models.generate_content(
                model=MODEL_PART2,
                contents=[article_content, PROMPT_PART2],
                config=types.GenerateContentConfig(
                    response_mime_type="application/json",
...
if __name__ == "__main__":
    ...
    target_filename = "trail.txt"
    target_path = os.path.join(INPUT_TXT_DIR, target_filename)

    # Check if this specific file exists
    if not os.path.exists(target_path):
        print(f"❌ Error: The file {target_filename} was not found in {INPUT_TXT_DIR}")
        exit()

    # Create a list containing only this single file
    txt_files = [target_path]

    print(f"Targeting single file: {target_filename}")

    for i, txt in enumerate(txt_files, 1):
        process_single_txt(client, txt)
